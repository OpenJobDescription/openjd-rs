...
fn test_attr_os_family_all_of_two_literals_plus_expr() {
    ...
    // under every possible resolution: an expression element may null-skip
    // or flatten, but a literal always contributes exactly one resolved
    // element. Rejected at decode even though the list is not all-literal.
    let v = yaml_val(
        r#"{
        "specificationVersion": "jobtemplate-2023-09",
        "name": "Test",
...
fn test_attr_os_family_all_of_one_literal_plus_expr_deferred() {
    // One literal plus one expression may still resolve to a single
    // element (the expression can null-skip), so decode defers the
    // single-valued count to job creation.
    let v = yaml_val(
        r#"{
        "specificationVersion": "jobtemplate-2023-09",
        "name": "Test",
