...
1. ~~**Fix `hash_file_chunked` zero-chunk-size hang.** Reject `chunk_size == 0` (and arguably any non-positive value except `WHOLE_FILE_CHUNK_SIZE`, which isn't a valid input to this function anyway). Add a unit test. Update the doc comment. The new probe test can be un-ignored once fixed.~~ **Resolved** — `hash_file_chunked` now returns `InvalidInput` for `chunk_size == 0`; unit test `hash_chunked_rejects_zero_chunk_size` added; doc comment updated.
...
3. ~~**Clean up `hash_file_chunked` dead code.** The `UnexpectedEof` branch allocates `tail` via `read_to_end` then discards it, seeks back, and re-reads. Simplify to a single `read_to_end` into the remainder, or compute the remainder length from file metadata.~~ **Resolved** — `hash_file_chunked` now takes an `expected_size: u64` parameter, verifies it against `file.metadata()?.len()` (returning `InvalidData` on mismatch), and walks full chunks + one final partial chunk computed from the verified size. No more seek/re-read. Unit tests `hash_chunked_size_mismatch_longer_on_disk` and `hash_chunked_size_mismatch_shorter_on_disk` added.
4. ~~**De-duplicate the duplicate-path check** between `Manifest::validate` and `codec::check_no_duplicate_paths`. After constructing a typed manifest in the decode path, call `validate()` rather than re-checking a subset inline. If performance is the concern, factor the check into a single shared helper.~~ **Resolved** — `codec::check_no_duplicate_paths` removed; `decode_v2023` and `decode_v2025` now call `manifest.validate()` on the typed result. One pre-existing test with an invalid chunkhashes-without-chunking combination was corrected.
5. ~~**Reduce `AsyncDataCache` surface area.** Split multipart/range operations into a separate trait (`MultipartDataCache`, `RangeReadDataCache`) so `FileSystemDataCache` doesn't carry unused-error stubs. `cache_sync` can then condition its fast path on trait presence.~~ **Resolved** — `AsyncDataCache` is now a core trait; multipart and byte-range methods live on new `MultipartDataCache` and `RangeReadDataCache` traits. `AsyncDataCache` exposes `as_multipart()`/`as_range_read()` accessors (default `None`) for capability discovery. `FileSystemDataCache` no longer carries `Unsupported`-returning stubs. `ops/hash_upload.rs`, `ops/download.rs`, and `ops/cache_sync.rs` branch on the accessors. Public API signatures unchanged.
6. ~~**Wrap `glob::PatternError` in `SnapshotError::Validation`.** `IncludeExcludePathsFilter::new` leaks a dependency into the public API. Make it return `crate::Result<Self>`.~~ **Resolved** — `IncludeExcludePathsFilter::new` now returns `crate::Result<Self>` with a `Validation` error on bad glob patterns.
7. ~~**Make `S3DataCache` fields private** behind builder or `with_*` setters. Current exposed fields bind the semver contract to implementation details.~~ **Resolved** — all `S3DataCache` fields are now private. Added consuming builder setters `with_multipart_part_size`, `with_s3_check_cache`, `with_force_s3_check`, `with_expected_bucket_owner`; added read accessors `client()`, `expected_bucket_owner()`, `is_cache_validation_invalidated()`.
