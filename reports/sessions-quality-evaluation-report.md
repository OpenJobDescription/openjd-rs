...
| 8 | ~~`session.md` §Enter~~ | ~~Shows signature `enter_environment(env, resolved_symtab, identifier, os_env_vars)`~~ | ~~Actual also takes `identifier` as `Option<&str>` and returns `SessionError`. Resolved_symtab is `Option<&SerializedSymbolTable>` — not noted as optional in prose.~~ **Resolved** — §Enter now shows the return type and notes the optional arguments. |
